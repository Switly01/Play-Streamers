(() => {
  ...
  const money=(n,c='TRY')=>(c==='TRY'?'₺':c+' ')+Number(n||0).toLocaleString('tr-TR',{maximumFractionDigits:2}); const donateDt=t=>new Intl.DateTimeFormat('tr-TR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date(t||Date.now())); const id=()=>Math.random().toString(36).slice(2)+Date.now().toString(36); const hue=s=>[...String(s)].reduce((a,c)=>(a*31+c.charCodeAt(0))%360,0); const kickProfileUrl=()=>state.settings.kickAccount?.username?`https://kick.com/${encodeURIComponent(state.settings.kickAccount.username)}`:'';
  ...
  async function completeSwIdentityCallback(){const url=new URL(location.href);if(url.pathname!=='/identity/callback')return false;const code=String(url.searchParams.get('code')||''),returnedState=String(url.searchParams.get('state')||''),expectedState=String(sessionStorage.getItem('ps-sw-identity-state')||'');if(!expectedState||returnedState!==expectedState){history.replaceState(null,'','/');notice('SW Identity doğrulanamadı','Giriş isteğinin güvenlik kodu eşleşmedi. Lütfen yeniden dene.','!');return true}if(url.searchParams.get('two_factor_required')==='1'){const challengeId=String(url.searchParams.get('challenge_id')||'');history.replaceState(null,'','/');showLandingAuthV101('login');const form=$('#landingAuthModal .ps-identity-credential-form');showInlineTwoFactor(form,challengeId,identityProductRequest(returnedState));return true}if(!/^[a-f0-9]{64}$/i.test(code)){history.replaceState(null,'','/');notice('SW Identity doğrulanamadı','Tek kullanımlık giriş kodu alınamadı. Lütfen yeniden dene.','!');return true}sessionStorage.removeItem('ps-sw-identity-state');try{const response=await fetch(`${API_BASE}/api/auth/sw/exchange`,{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({code,redirectUri:SW_IDENTITY_REDIRECT})});const data=await response.json().catch(()=>({}));if(!response.ok||!data.sessionId)throw new Error(data.error||'SW Identity girişi tamamlanamadı.');landingMode=false;activeUserSession=data.sessionId;adoptAuthenticatedUser(data.user);state.settings.userSession=activeUserSession;save();history.replaceState(null,'','/home');render();notice('SW Identity bağlandı',`${data.plan?.label||'Free'} planın ve hesabın eşitlendi.`,'✓');window.dispatchEvent(new Event('ps-route-change'))}catch(error){history.replaceState(null,'','/');notice('Giriş tamamlanamadı',error.message||'SW Identity şu anda yanıt vermedi.','!')}return true}
  ...
  function showLandingAuthV101(mode){
    ...
    $('[data-provider="google"]',layer).onclick=()=>startSwIdentityLogin('google');$('[data-provider="kick"]',layer).onclick=()=>startSwIdentityLogin('kick');$('[data-provider="sw"]',layer)?.addEventListener('click',()=>{const field=form.elements.identity;form.classList.remove('is-sw-selected');void form.offsetWidth;form.classList.add('is-sw-selected');field?.focus()});if(!isLogin)installIdentityCalendar(form.elements.birthDate);
  }
  ...
  function renderLiveStatus(){installLiveStatus();const box=$('#streamStatus');box.classList.toggle('live',liveInfo.state==='live');box.classList.toggle('offline',liveInfo.state==='offline');let dot=$(':scope > .dot',box);if(!dot){dot=document.createElement('span');dot.className='dot';box.prepend(dot)}let message=$(':scope > [data-live-message]',box);if(!message){message=document.createElement('span');message.dataset.liveMessage='1';dot.after(message)}if(message.textContent!==liveInfo.message)message.textContent=liveInfo.message;let clear=$('#clearBtn',box)||document.getElementById('clearBtn');if(!clear){clear=document.createElement('button');clear.className='clear';clear.id='clearBtn';clear.type='button';clear.textContent='Yayıncı paneli verilerini sıfırla'}if(clear.parentElement!==box)box.append(clear)}
  ...
  if(location.pathname==='/identity/callback')void completeSwIdentityCallback();else void refreshUserSession();
})();
...
(() => {
  ...
  function localeMenu(button){
    ...
    $$('[data-ps15-lang]',menu).forEach(item=>item.onclick=()=>{ const selected=item.dataset.ps15Lang; localStorage.setItem('ps15-locale',selected); loader(true); menuClose(menu); setTimeout(()=>location.reload(),180); });
  }
