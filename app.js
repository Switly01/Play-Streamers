(() => {
  ...
  let legacySettingsRemoved=false; if('viewerSamples' in state.stats){delete state.stats.viewerSamples;legacySettingsRemoved=true} ['setupComplete','kickChannelUrl','donationAccounts'].forEach(key=>{if(key in state.settings){delete state.settings[key];legacySettingsRemoved=true}}); activeKickSession=state.settings.kickSession||''; activeUserSession=state.settings.userSession||''; if(activeUserSession)landingMode=false; const $=(s,root=document)=>root.querySelector(s); const $$=(s,root=document)=>[...root.querySelectorAll(s)];
  const save=()=>localStorage.setItem(KEY,JSON.stringify(state)); if(legacySettingsRemoved)save(); const oauthResult=new URLSearchParams(location.hash.slice(1)); let kickJustConnected=false; let googleJustConnected=false; if(oauthResult.get('kick_connected')==='1'&&oauthResult.get('kick_session')){activeKickSession=oauthResult.get('kick_session');state.settings.kickSession=activeKickSession;kickJustConnected=true}if(oauthResult.get('google_connected')==='1'&&oauthResult.get('user_session')){activeUserSession=oauthResult.get('user_session');state.settings.userSession=activeUserSession;googleJustConnected=true}if(googleJustConnected&&resumeAccountFlow){landingMode=false;sessionStorage.removeItem('play-streamers-account-flow')}if(kickJustConnected||googleJustConnected){save();history.replaceState(null,'',location.pathname+location.search)} const esc=s=>{const d=document.createElement('div');d.textContent=String(s??'');return d.innerHTML}; const month=()=>new Date().toISOString().slice(0,7); const interfaceLocale=()=>({tr:'tr-TR',en:'en-US',de:'de-DE',es:'es-ES',fr:'fr-FR',ru:'ru-RU',ar:'ar-SA',ja:'ja-JP'})[String(localStorage.getItem('ps15-locale')||document.documentElement.lang||'tr').split('-')[0]]||'en-US'; const localeCurrency=()=>({tr:'TRY',en:'USD',de:'EUR',es:'EUR',fr:'EUR',ru:'RUB',ar:'SAR',ja:'JPY'})[String(localStorage.getItem('ps15-locale')||document.documentElement.lang||'tr').split('-')[0]]||'USD'; const dt=t=>new Intl.DateTimeFormat(interfaceLocale(),{dateStyle:'short',timeStyle:'short'}).format(new Date(t||Date.now()));
  const FX_CACHE_KEY='ps129-exchange-rates'; const DONATE_VISIBLE_AGE_MS=24*60*60*1000; let exchangeRates=null; try{const cached=JSON.parse(localStorage.getItem(FX_CACHE_KEY)||'null');if(cached?.rates&&Date.now()-Number(cached.savedAt||0)<1800000)exchangeRates=cached}catch{}
  const convertMoney=(amount,source,target=localeCurrency())=>{const from=String(source||target).toUpperCase(),to=String(target||localeCurrency()).toUpperCase(),rates=exchangeRates?.rates||{};if(from===to)return Number(amount||0);if(!Number.isFinite(Number(rates[from]))||!Number.isFinite(Number(rates[to])))return null;return Number(amount||0)/Number(rates[from])*Number(rates[to])};
  const money=(n,c='')=>{const source=String(c||localeCurrency()).toUpperCase(),target=localeCurrency(),converted=convertMoney(n,source,target);return new Intl.NumberFormat(interfaceLocale(),{style:'currency',currency:converted===null?source:target,maximumFractionDigits:2}).format(converted===null?Number(n||0):converted)}; const donateDt=t=>new Intl.DateTimeFormat(interfaceLocale(),{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date(t||Date.now())); const id=()=>Math.random().toString(36).slice(2)+Date.now().toString(36); const hue=s=>[...String(s)].reduce((a,c)=>(a*31+c.charCodeAt(0))%360,0); const kickProfileUrl=()=>state.settings.kickAccount?.username?`https://kick.com/${encodeURIComponent(state.settings.kickAccount.username)}`:'';
  function publishExchangeRates(){window.psExchangeRates=exchangeRates;window.dispatchEvent(new CustomEvent('ps:exchange-rates',{detail:exchangeRates||{}}))}
  async function loadExchangeRates(force=false){if(!force&&exchangeRates&&Date.now()-Number(exchangeRates.savedAt||0)<300000){publishExchangeRates();return exchangeRates}try{const url=`${API_BASE}/api/public/exchange-rates${force?`?refresh=${Date.now()}`:''}`,response=await fetch(url,{cache:'no-store',credentials:'omit'}),data=await response.json();if(!response.ok||!data?.rates)throw Error('rates');exchangeRates={...data,savedAt:Date.now()};localStorage.setItem(FX_CACHE_KEY,JSON.stringify(exchangeRates));publishExchangeRates();return exchangeRates}catch{publishExchangeRates();return exchangeRates}}
  window.psReloadExchangeRates=()=>loadExchangeRates(true);
  window.setInterval(()=>{if(document.visibilityState==='visible'&&navigator.onLine!==false)loadExchangeRates(true)},300000);
  ...
  function renderKickConnectionStatus(){const row=$('#connectionList .con-row');if(!row)return;const account=state.settings.kickAccount;const small=$('small',row);const button=$('.refresh',row);const connected=Boolean(activeKickSession);if(small)small.textContent=connected?(account?.username?`Bağlı · @${account.username}`:kickConnectionStatus):'Giriş yapılmadı';if(button){button.textContent=connected?'⟳':'↗';button.title=connected?'Bağlantıyı yenile':'Kick ile giriş yap';button.onclick=()=>connected?refreshKickStatus(true):startKickLogin()}}
  ...
  function refreshInterfaceLanguage(){window.dispatchEvent(new Event('ps:i18n-refresh'));window.requestAnimationFrame(()=>window.psLiveI18n?.refresh?.())}
  ...
  function completeSignIn(data,{emailMissing=false}={}){landingMode=false;activeUserSession=data.sessionId||activeUserSession;adoptAuthenticatedUser(data.user);state.settings.userSession=activeUserSession;if(data.plan)state.settings.plan=data.plan;save();removeLandingAuth();render();refreshInterfaceLanguage();if(emailMissing)showEmailMissingNotice()}
  ...
  async function submitSwIdentityCredentials(form,isLogin,startedAt){
    const error=$('.ps-identity-form-error',form),submit=$('.ps-identity-form-submit',form);
  ...
  function installIdentityCalendar(input){
    ...
    const calendar=document.createElement('section');calendar.className='ps102-calendar';calendar.hidden=true;calendar.setAttribute('aria-label','Doğum tarihi takvimi');field.append(calendar);
    const maximum=new Date(`${adultBirthDate()}T12:00:00`);let cursor=new Date(maximum.getFullYear(),maximum.getMonth(),1);
    const formatIso=date=>`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;
    const draw=()=>{const year=cursor.getFullYear(),month=cursor.getMonth(),first=(new Date(year,month,1).getDay()+6)%7,last=new Date(year,month+1,0).getDate(),monthLabel=new Intl.DateTimeFormat('tr-TR',{month:'long',year:'numeric'}).format(cursor);calendar.innerHTML=`<header><button type="button" data-cal-prev aria-label="Önceki ay">←</button><strong>${monthLabel}</strong><button type="button" data-cal-next aria-label="Sonraki ay">→</button></header><div class="ps102-calendar-week"><span>Pt</span><span>Sa</span><span>Ça</span><span>Pe</span><span>Cu</span><span>Ct</span><span>Pa</span></div><div class="ps102-calendar-grid">${'<i></i>'.repeat(first)}${Array.from({length:last},(_,index)=>{const date=new Date(year,month,index+1),iso=formatIso(date),disabled=date>maximum;return `<button type="button" data-date="${iso}" ${disabled?'disabled':''} class="${input.value===iso?'is-selected':''}">${index+1}</button>`}).join('')}</div>`;calendar.querySelector('[data-cal-prev]').onclick=()=>{cursor=new Date(year,month-1,1);draw()};const next=calendar.querySelector('[data-cal-next]');next.disabled=new Date(year,month+1,1)>maximum;next.onclick=()=>{cursor=new Date(year,month+1,1);draw()};calendar.querySelectorAll('[data-date]').forEach(button=>button.onclick=()=>{input.value=button.dataset.date;input.dispatchEvent(new Event('change',{bubbles:true}));calendar.hidden=true;field.classList.remove('is-calendar-open')})};
    const open=event=>{event.preventDefault();calendar.hidden=!calendar.hidden;field.classList.toggle('is-calendar-open',!calendar.hidden);if(!calendar.hidden)draw()};input.addEventListener('click',open);input.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '||event.key==='ArrowDown')open(event);if(event.key==='Escape'){calendar.hidden=true;field.classList.remove('is-calendar-open')}});document.addEventListener('pointerdown',event=>{if(!field.contains(event.target)){calendar.hidden=true;field.classList.remove('is-calendar-open')}},{capture:true});draw();
  }
  function showLandingAuthV101(mode){
    window.ps32ReleaseTurnstileHost?.();
    $('#landingAuthModal')?.remove();
    const isLogin=mode==='login',startedAt=Date.now();
    const layer=document.createElement('div');layer.className='landing-auth-modal ps-identity-auth';layer.id='landingAuthModal';
  ...
  function installTopMenu(){const oldInfo=$('#updatesBtn');if(oldInfo)oldInfo.style.display='none';const connection=$('#connectionBtn');if(connection){connection.textContent='📶';connection.title='Bağlantı durumu'}if(!$('#menuBtn')){const button=document.createElement('button');button.className='hamburger-menu';button.id='menuBtn';button.type='button';button.title='Menü';button.textContent='☰';$('.actions').append(button);const menu=document.createElement('aside');menu.className='side-menu';menu.id='sideMenu';menu.hidden=true;menu.innerHTML='<button id="menuUpdates">Güncelleme notları</button><button id="menuConnections">Bağlantı durumu</button>';document.body.append(menu);button.onclick=()=>{menu.hidden=!menu.hidden};$('#menuUpdates').onclick=()=>{menu.hidden=true;showUpdates()};$('#menuConnections').onclick=()=>{menu.hidden=true;$('#connections').hidden=false;renderConnectionHub()}}const foot=$('.foot');if(foot)foot.innerHTML='<a href="https://swcreate.com" target="_blank" rel="noopener noreferrer">Developed by SW CREATE</a>'}
  function eventMessage(v){return esc(v||'').replace(/(https?:\/\/[^\s<]+)/g,'<a href="$1" target="_blank" rel="noopener noreferrer">$1</a>')}
  function listCard(key,title,cls,rows,mapper,emptyText,count){return `<section class="card ${cls}" data-card="${key}"><div class="card-body"><div class="card-head"><div class="title"><span class="logo ${key==='donations'?'donate':''}">${key==='donations'?'₺':'K'}</span><h2>${title}</h2></div><button class="expand">⛶</button><div class="count">${count}</div></div><ul class="entries" id="list-${key}">${rows.map(mapper).join('')}</ul><div class="empty ${rows.length?'':'show'}"${rows.length?' hidden':''}>${emptyText}</div></div></section>`}
  function itemHtml(item,kind){const icon=item.sourceLogo?`<span class="source-badge has-image"><img src="${esc(item.sourceLogo)}" alt="${esc(item.sourceName||'Donate')} logosu"></span>`:(item.sourceIcon?`<span class="source-badge">${esc(item.sourceIcon)}</span>`:'');const badge=kind==='subs'?'<span class="sub-badge">K Abone</span>':'';const accent=kind==='gifts'?`style="border-color:hsl(${hue(item.name)} 85% 66%)"`:'';const expand=kind==='donations'?'':'<button class="event-expand" type="button" aria-label="Olayı büyüt">⛶</button>';return `<li class="${item.read?'read':'new'}" ${accent} data-kind="${kind}" data-id="${esc(item.id)}"><div class="event-main"><div class="event-top">${icon}<span class="name">${esc(item.name)}</span>${badge}<span class="meta">${esc(item.meta)}</span></div><div class="message">${eventMessage(item.message)}</div></div>${expand}</li>`}
  function panel(){state.events.followers??=[];state.totals.followers??=state.events.followers.length;const followers=state.events.followers,subs=Object.values(state.events.subs).filter(x=>!x.expiresAt||!Number.isFinite(Date.parse(x.expiresAt))||Date.parse(x.expiresAt)>=Date.now());const one=subs.filter(x=>x.months===1).sort((a,b)=>b.at-a.at),multi=subs.filter(x=>x.months>=2).sort((a,b)=>b.at-a.at);const kicks=state.events.kicks,gifts=state.events.gifts,dons=state.events.donations;const mapFollower=x=>itemHtml({id:x.id,name:x.name,meta:dt(x.at),message:x.message,read:x.read,at:x.at,sourceUrl:x.sourceUrl},'followers');const mapSub=x=>itemHtml({id:x.id,name:x.name,meta:`${x.months} ay · ${dt(x.at)}`,message:x.message,read:x.read,at:x.at,sourceUrl:x.sourceUrl},'subs');const mapKick=x=>itemHtml({id:x.id,name:x.name,meta:`${x.amount} kicks · ${dt(x.at)}`,message:x.message,read:x.read,at:x.at,sourceUrl:x.sourceUrl},'kicks');const mapGift=x=>itemHtml({id:x.id,name:x.name,meta:`${x.count} hediye · ${dt(x.at)}`,message:x.message,read:x.read,at:x.at,sourceUrl:x.sourceUrl},'gifts');const mapDon=x=>itemHtml({id:x.id,name:x.name,meta:`${money(x.amount,x.currency)} · ${donateDt(x.at)}`,message:x.message,read:x.read,at:x.at,sourceUrl:x.sourceUrl,sourceIcon:x.sourceIcon,sourceLogo:x.sourceLogo,sourceName:x.sourceName},'donations');const donationTotal=dons.reduce((sum,item)=>sum+(convertMoney(item.amount,item.currency||'TRY',localeCurrency())||0),0);$('#panelGrid').innerHTML=[listCard('followers','Takipçi','cyan',followers,mapFollower,'Yeni takipçiler burada görünür.',state.totals.followers),listCard('onemonth','1 Aylık Abone','violet',one,mapSub,'İlk ayındaki aktif aboneler burada görünür.',one.length),listCard('multimonth','2+ Aylık Abone','magenta',multi,mapSub,'İki ay ve üzerindeki aktif aboneler burada görünür.',multi.length),listCard('gifts','Hediye Abonelik','cyan',gifts,mapGift,'Hediye abonelikler burada görünür.',state.totals.gifts),listCard('kicks','Kicks','lime',kicks,mapKick,'Kicks olayları burada görünür.',state.totals.kicks),listCard('donations','Donate','amber donate',dons,mapDon,'Bağış olayları burada görünür.',money(donationTotal,localeCurrency()))].join('');bindPanel();const t=[...followers.slice(0,3).map(x=>`＋ ${x.name} — yeni takipçi`),...dons.slice(0,4).map(x=>`💰 ${x.name} — ${money(x.amount,x.currency)}`),...kicks.slice(0,4).map(x=>`⚡ ${x.name} — ${x.amount} kicks`),...gifts.slice(0,3).map(x=>`🎁 ${x.name} — ${x.count} hediye`)];$('#ticker').textContent=t.length?t.join('     •     '):'Henüz olay yok — veri kaynağı bağlandığında burada akacak.'}
  function bindPanel(){$$('.expand').forEach(b=>b.onclick=()=>{const c=b.closest('.card');const on=!c.classList.contains('expanded');$$('.card.expanded').forEach(x=>x.classList.remove('expanded'));if(on)c.classList.add('expanded')});$$('.entries li').forEach(li=>{const kind=li.dataset.kind,item=findEvent(kind,li.dataset.id);li.onclick=e=>{if(e.target.closest('a')||e.target.closest('.event-expand'))return;sessionStorage.setItem('ps-second-dashboard','1');markRead(kind,item,li);openEvent(item,kind)};const expand=li.querySelector('.event-expand');if(expand)expand.onclick=e=>{e.stopPropagation();sessionStorage.setItem('ps-second-dashboard','1');markRead(kind,item,li);openEvent(item,kind)}})}
  ...
  function markRead(kind,item,row){syncDashboardState();item=findEvent(kind,item?.id)||item;if(!item||item.read)return;item.read=true;item.readAt=Date.now();save();if(row){row.classList.remove('new');row.classList.add('read')}scheduleRemove(kind,item)}
  function removeRenderedEvent(kind,item){$$('.entries li').forEach(row=>{if(row.dataset.kind===kind&&row.dataset.id===String(item.id))row.remove()});$$('.entries').forEach(list=>{const empty=list.parentElement?.querySelector('.empty');if(empty)empty.classList.toggle('show',!list.querySelector('li'))})}
  function scheduleRemove(kind,item){const k=kind+item.id;if(removalTimers.has(k))return;removalTimers.add(k);const wait=Math.max(0,60000-(Date.now()-(item.readAt||Date.now())));setTimeout(()=>{syncDashboardState();if(kind==='subs')delete state.events.subs[item.name.toLowerCase()];else state.events[kind]=state.events[kind].filter(x=>x.id!==item.id);save();removalTimers.delete(k);removeRenderedEvent(kind,item)},wait)}
  function openEvent(item,kind){if(!item)return;const t=source=>window.psTranslateInterface?.(source)||source;const donate=kind==='donations';const sourceUrl=String(item.sourceUrl||'').trim();const hasSource=/^https?:\/\//i.test(sourceUrl);const sourceAction=!donate&&hasSource?`<a href="${esc(sourceUrl)}" target="_blank" rel="noopener noreferrer">Kick profilini aç ↗</a>`:(!donate?'<span class="event-source-unavailable">Kaynak bağlantısı yok</span>':'');const sourceBrand=donate?`<div class="event-source-brand">${item.sourceLogo?`<img src="${esc(item.sourceLogo)}" alt="${esc(item.sourceName||'Donate')} logosu">`:''}<span>${esc(item.sourceName||'Donate')}</span></div>`:'';const eventMeta=donate?`${money(item.amount,item.currency)} · ${donateDt(item.at)}`:(item.meta||dt(item.at));const donateDetails=donate?`<dl class="event-detail-grid"><div><dt>${esc(t('Gönderen'))}</dt><dd>${esc(item.name||'Anonim')}</dd></div><div><dt>${esc(t('Tutar'))}</dt><dd>${esc(money(item.amount,item.currency))}</dd></div><div><dt>${esc(t('Tarih ve saat'))}</dt><dd>${esc(donateDt(item.at))}</dd></div><div><dt>${esc(t('Platform'))}</dt><dd>${esc(item.sourceName||'Donate')}</dd></div></dl><section class="event-detail-message"><span>${esc(t('Mesaj'))}</span><p>${eventMessage(item.message||t('Mesaj bırakılmadı.'))}</p></section>`:'';const modal=$('#modal');document.body.append(modal);modal.classList.add('ps133-event-overlay');modal.hidden=false;$('#modal').innerHTML=`<section class="dialog${donate?' event-detail-dialog':''}"><button class="close" type="button" aria-label="${esc(t('Kapat'))}">×</button><span class="eyebrow">${esc(t(donate?'DONATE AYRINTISI':'OLAY AYRINTISI'))}</span>${sourceBrand}<h2>${esc(item.name)}</h2>${donate?donateDetails:`<div class="event-message">${eventMessage(item.message)}</div><div class="event-foot"><span>${esc(eventMeta)}</span>${sourceAction}</div>`}</section>`;$('#modal .close').onclick=closeModal;$('#modal').onclick=e=>{if(e.target===$('#modal'))closeModal}}
  function closeModal(){$('#modal').hidden=true;$('#modal').innerHTML='';$('#modal').classList.remove('ps133-event-overlay')}
  ...
  function periodCard(prefix,stats,fmt){const currentMonth=new Date().toISOString().slice(0,7),stored=stats?.month||{},month=stored[currentMonth]||((Object.values(stored).some(row=>row&&typeof row.total==='number'))?stored:{});return `<div class="period-tabs"><button class="active" data-period="month">Bu ay</button><button data-period="all">Tüm zamanlar</button></div><div class="period-list" data-period-list="month">${rank(leaderboard(month),fmt)}</div><div class="period-list" data-period-list="all" hidden>${rank(leaderboard(stats.all),fmt)}</div>`}
  function syncDashboardState() {
    // Account and server-insight updates share this store but use another UI controller.
    // Read their latest state before a feed mutation so profile/settings/stats cannot be rolled back.
    try {
      const saved = JSON.parse(localStorage.getItem(KEY) || 'null');
      if (saved?.settings && saved?.events && saved?.stats && saved?.totals) state = saved;
    } catch (_) { /* Keep the valid in-memory state when storage is unavailable. */ }
  }
  function ensureDonationCurrencyHistory() {
    // Currency totals belong to statistics, never to the disposable panel feed.
    if (state.stats.donationCurrencies) return;
    state.stats.donationCurrencies = { month: {}, all: {} };
    const boundary = Number(state.settings.statsResetAt || 0);
    state.events.donations.filter(row => Number(row.at) >= boundary).forEach(addDonationCurrencyStat);
    save();
  }
  function addDonationCurrencyStat(row) {
    const history = state.stats.donationCurrencies;
    const at = Number(row.at), monthKey = new Date(at).toISOString().slice(0, 7);
    const key = String(row.name || 'Destekçi').toLowerCase(), currency = String(row.currency || 'TRY').toUpperCase();
    const add = bucket => {
      bucket[key] ??= { name: row.name || 'Destekçi', at, amounts: {} };
      bucket[key].amounts[currency] = Number(bucket[key].amounts[currency] || 0) + Number(row.amount || 0);
      bucket[key].at = Math.max(bucket[key].at, at);
    };
    add(history.all);
    history.month[monthKey] ??= {};
    add(history.month[monthKey]);
  }
  function convertedDonationStats() {
    ensureDonationCurrencyHistory();
    const target = localeCurrency(), history = state.stats.donationCurrencies;
    const convertBucket = bucket => Object.fromEntries(Object.entries(bucket).flatMap(([key, row]) => {
      const amounts = Object.entries(row.amounts).map(([currency, amount]) => convertMoney(amount, currency, target));
      if (amounts.some(value => value === null)) return [];
      return [[key, { name: row.name, at: row.at, currency: target, total: amounts.reduce((sum, amount) => sum + amount, 0) }]];
    }));
    return { all: convertBucket(history.all), month: Object.fromEntries(Object.entries(history.month).map(([key, bucket]) => [key, convertBucket(bucket)])) };
  }
  function stats(){const cutoff=Date.now()-30*24*60*60*1000,joinedByName={};Object.values(state.stats.joined||{}).forEach(bucket=>Object.entries(bucket||{}).forEach(([key,row])=>{if(Number(row?.at||0)>=cutoff&&(!joinedByName[key]||Number(row.at)>Number(joinedByName[key].at||0)))joinedByName[key]=row}));const joined=Object.values(joinedByName).sort((a,b)=>b.at-a.at).slice(0,10);const sub=leaderboard(state.stats.subs),donationStats=convertedDonationStats();$('#statsGrid').innerHTML=[statCard('Yeni Katılanlar','violet',`<p class="stats-desc">Son 30 günde ilk kez abone olan kişiler.</p>${rank(joined,()=> 'Yeni abone')}`),statCard('Top Abone','magenta',`<p class="stats-desc">Toplam aylık aboneliği en yüksek ilk 10 kişi.</p>${rank(sub,x=>x.total+' ay')}`),statCard('Top Hediye Abonelik','cyan',periodCard('gift',state.stats.gifts,x=>x.total+' hediye')),statCard('Top Kicks','lime',periodCard('kick',state.stats.kicks,x=>x.total+' kicks')),statCard('Top Donate','amber',periodCard('donate',donationStats,x=>money(x.total,localeCurrency())),true)].join('');$$('.period-tabs button').forEach(b=>b.onclick=()=>{const p=b.dataset.period,box=b.closest('.card');$$('.period-tabs button',box).forEach(x=>x.classList.toggle('active',x===b));$$('.period-list',box).forEach(x=>x.hidden=x.dataset.periodList!==p)})}
  function addStat(type,name,amount,at){const key=name.toLowerCase();const set=(bucket)=>{bucket[key]??={name,total:0,at};bucket[key].total+=amount;bucket[key].at=at};if(type==='subs'){state.stats.subs[key]??={name,total:0,at};state.stats.subs[key].total=Math.max(Number(state.stats.subs[key].total||0),Number(amount||1));state.stats.subs[key].at=at;if(Number(amount)===1){const firstSeen=Object.values(state.stats.joined||{}).some(bucket=>Boolean(bucket?.[key]));if(!firstSeen){const m=new Date(at).toISOString().slice(0,7);state.stats.joined[m]??={};state.stats.joined[m][key]={name,at}}}}else{const target=state.stats[type];set(target.all);const eventMonth=new Date(at).toISOString().slice(0,7);target.month[eventMonth]??={};const b=target.month[eventMonth];b[key]??={name,total:0,at};b[key].total+=amount;b[key].at=at}}
  function render(){panel();stats();renderConnectionHub();renderKickConnectionStatus();renderUserAuth();renderLiveStatus()}
  function installLiveStatus(){if($('#streamStatus'))return;const status=document.createElement('section');status.className='live-status';status.id='streamStatus';status.innerHTML='<span class="dot" aria-hidden="true"></span><span data-live-message="1">Yayın durumu hazırlanıyor</span>';$('#panelView').before(status)}
  function renderLiveStatus(){installLiveStatus();const box=$('#streamStatus'),isLive=liveInfo.state==='live',isOffline=liveInfo.state==='offline';box.classList.toggle('live',isLive);box.classList.toggle('offline',isOffline);box.classList.toggle('is-live',isLive);box.classList.toggle('is-offline',isOffline);let dot=$(':scope > .dot',box);if(!dot){dot=document.createElement('span');dot.className='dot';dot.setAttribute('aria-hidden','true');box.prepend(dot)}dot.title=isLive?'Yayın açık':isOffline?'Yayın kapalı':'Yayın durumu bekleniyor';let message=$(':scope > [data-live-message]',box);if(!message){message=document.createElement('span');message.dataset.liveMessage='1';dot.after(message)}$$(':scope > span:not(.dot):not([data-live-message])',box).forEach(node=>node.remove());if(message.textContent!==liveInfo.message)message.textContent=liveInfo.message;let clear=$('#clearBtn',box)||document.getElementById('clearBtn');if(!clear){clear=document.createElement('button');clear.className='clear';clear.id='clearBtn';clear.type='button';clear.textContent='Yayıncı paneli verilerini sıfırla'}if(clear.parentElement!==box)box.append(clear);window.dispatchEvent(new Event('ps:i18n-refresh'))}
  ...
  $('#connectionBtn').onclick=()=>{$('#connections').hidden=!$('#connections').hidden};$('#clearBtn').onclick=()=>window.ps53RequestPanelReset?.();
  ...
  window.PlayStreamers={addEvent(event){syncDashboardState();ensureDonationCurrencyHistory();const at=Number(event.at)||Date.now(),type=event.type,panelAllowed=at>=Number(state.settings.panelResetAt||0),statsAllowed=at>=Number(state.settings.statsResetAt||0);state.events.followers??=[];state.totals.followers??=state.events.followers.length;if(type==='follower'){const row={id:event.id||id(),name:event.name,message:event.message||`${event.name} takip etti.`,at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};if(panelAllowed){state.events.followers.unshift(row);state.totals.followers+=1}}if(type==='subscription'){const key=event.name.toLowerCase(),old=state.events.subs[key],months=Math.max(1,Number(event.months||1)),row={id:event.id||id(),name:event.name,months:Math.max(Number(old?.months||0),months),expiresAt:event.expiresAt||old?.expiresAt||'',message:event.message||`${event.name} abone oldu.`,at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};if(panelAllowed)state.events.subs[key]=row;if(statsAllowed)addStat('subs',event.name,row.months,at)}if(type==='kicks'){const row={id:event.id||id(),name:event.name,amount:+event.amount||0,message:event.message||'',at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};if(panelAllowed){state.events.kicks.unshift(row);state.totals.kicks+=row.amount}if(statsAllowed){addStat('kicks',row.name,row.amount,at)}}if(type==='gift'){const row={id:event.id||id(),name:event.name,count:+event.count||1,message:event.message||'',at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};if(panelAllowed){state.events.gifts.unshift(row);state.totals.gifts+=row.count}if(statsAllowed){addStat('gifts',row.name,row.count,at)}}if(type==='donation'){const row={id:event.id||id(),name:event.name,amount:+event.amount||0,currency:event.currency||localeCurrency(),message:event.message||'',sourceIcon:event.sourceIcon||new Intl.NumberFormat(interfaceLocale(),{style:'currency',currency:localeCurrency(),currencyDisplay:'narrowSymbol'}).formatToParts(0).find(part=>part.type==='currency')?.value||localeCurrency(),sourceName:event.sourceName||'Donate',sourceLogo:event.sourceLogo||'',providerId:event.providerId||'',sourceUrl:event.sourceUrl||'',at,read:false};if(panelAllowed){state.events.donations.unshift(row);state.totals.donations+=row.amount}if(statsAllowed){addStat('donations',row.name,row.amount,at);addDonationCurrencyStat(row)}}save();render()},resetPanel(){syncDashboardState();ensureDonationCurrencyHistory();state.events={followers:[],subs:{},kicks:[],gifts:[],donations:[]};state.totals={followers:0,kicks:0,donations:0,gifts:0};state.settings.panelResetAt=Date.now();save();render()},resetStats(){syncDashboardState();state.stats={subs:{},joined:{},gifts:{month:{},all:{}},kicks:{month:{},all:{}},donations:{month:{},all:{}}};state.stats.donationCurrencies={month:{},all:{}};state.settings.statsResetAt=Date.now();save();render()},renderDashboard(){syncDashboardState();render()},startBroadcast(){state.events={followers:[],subs:{},kicks:[],gifts:[],donations:[]};state.totals={followers:0,kicks:0,donations:0,gifts:0};state.stats={subs:{},joined:{},gifts:{month:{},all:{}},kicks:{month:{},all:{}},donations:{month:{},all:{}}};save();render()}};
  window.addEventListener('ps:locale-change',()=>{void loadExchangeRates();refreshInterfaceLanguage()});
  void loadExchangeRates();
  const installUserAuthControlsBase = installUserAuthControls;
  installUserAuthControls = function installCreatorUserAuthControls() {
    installUserAuthControlsBase();
    const overlay = $('#authOverlay'), hero = $('.landing-card', overlay), update = $('.landing-update-card', overlay);
    if (!overlay || !hero || !update) return;
    ...
    const heading = $('h1', hero); if (heading) heading.textContent = 'Yayınını planla. Veriyi anla. Büyümeyi tek yerden yönet.';
    ...
    const updateHeading = $('h2', update); if (updateHeading) updateHeading.textContent = '0.14.0 · Birleşik Premium Tasarım';
  };
...
(() => {
  ...
  function installCarousel(root) {
    ...
    host.innerHTML = `<section class="product-window" aria-label="Play Streamers temsili masaüstü arayüz demosu">
      <header class="product-top"><span>PLAY STREAMERS DESKTOP · TEMSİLİ ARAYÜZ</span><span class="product-dots"><i></i><i></i><i></i></span></header>
      <div class="product-carousel">
        <section class="product-page" data-page="0">
          <div class="preview-hero"><div><span class="preview-kicker">YAYINCI ÇALIŞMA ALANI</span><h3>Üretim akışını tek ekranda yönet.</h3><p>Bu alan ürün görünümünü tanıtan bir arayüz demosudur; canlı hesap verisi içermez.</p><div class="preview-live"><i></i>ARAYÜZ DEMOSU</div></div><div class="preview-stat-stack"><div><b>45</b><span>hazır araç</span></div><div><b>8</b><span>odaklı bölüm</span></div></div></div>
          <div class="preview-columns"><article class="preview-panel"><span class="preview-kicker">İÇERİK AKIŞI</span><h4>Yayın planı</h4><div class="preview-event"><i>01</i><b>Açılış notları</b><span>hazır</span></div><div class="preview-event"><i>02</i><b>Ana bölümler</b><span>hazır</span></div><div class="preview-event"><i>03</i><b>Kapanış hedefi</b><span>hazır</span></div></article><article class="preview-panel"><span class="preview-kicker">ETKİLEŞİM</span><h4>Doğrulanmış eğilim</h4><div class="preview-bars"><i style="height:28%"></i><i></i><i></i><i></i><i></i><i></i></div></article></div>
        </section>
        <section class="product-page" data-page="1" hidden>
          <div class="preview-hero"><div><span class="preview-kicker">DOĞRULANMIŞ CANLI VERİ</span><h3>Rakamın kaynağı da görünür.</h3><p>Topluluk ölçümleri yalnızca bağlı hesaptan doğrulanmış veri geldiğinde doldurulur.</p></div><div class="preview-stat-stack"><div><b>—</b><span>hesap bağlanınca</span></div><div><b>—</b><span>veri doğrulanınca</span></div></div></div>
          <div class="preview-columns"><article class="preview-panel"><span class="preview-kicker">TOPLULUK ÖZETİ</span><h4>Doğrulama bekleniyor</h4><div class="preview-rank"><div><span><b>Kick hesabı</b></span><strong>bağlanmadı</strong></div><div><span><b>Play Connect</b></span><strong>bağlanmadı</strong></div><div><span><b>Yayın oturumu</b></span><strong>kapalı</strong></div></div></article><article class="preview-panel"><span class="preview-kicker">EĞİLİM</span><h4>Veri geldiğinde oluşur</h4><div class="preview-bars"><i style="height:16%"></i><i style="height:16%"></i><i style="height:16%"></i><i style="height:16%"></i><i style="height:16%"></i><i style="height:16%"></i></div></article></div>
        </section>
        <section class="product-page" data-page="2" hidden>
          <div class="preview-donate"><div><span class="preview-kicker">CREATOR OS</span><strong>Üretim sistemin tek uygulamada.</strong><p>İçerik, topluluk, marka, gelir, analiz ve yerel kasa araçları aynı masaüstü akışında çalışır.</p></div><div class="preview-source-list"><span>Free <b>Hazır</b></span><span>Pro <b>Hazır</b></span><span>Product Pro <b>Hazır</b></span></div></div>
          <div class="preview-columns"><article class="preview-panel"><span class="preview-kicker">ÇALIŞMA ALANLARI</span><h4>45 hazır araç</h4><div class="preview-event"><i>İ</i><b>İçerik ve yayın planı</b><span>yerel</span></div><div class="preview-event"><i>T</i><b>Topluluk ve büyüme</b><span>doğrulanmış</span></div><div class="preview-event"><i>K</i><b>Marka ve yaratıcı kasa</b><span>güvenli</span></div></article><article class="preview-panel"><span class="preview-kicker">KONTROL</span><h4>Site sade, araçlar uygulamada.</h4><p style="color:#aec0d4;font-size:12px;line-height:1.6">Sitede hesap, plan ve bağlantıları; uygulamada içerik, analiz, topluluk, marka ve gelir araçlarını yönet.</p></article></div>
        </section>
  ...
  function enhanceLanding() {
    ...
    if (footer && !$('.switly-link', footer)) footer.lastElementChild.innerHTML = 'Developed by <a class="switly-link" href="https://swcreate.com">SW CREATE</a>';
...
(() => {
  ...
  function decorateDashboard() {
    const actions = $('.topbar .actions'); if (actions) { const account = $('#googleAccountBtn'); if (account) account.hidden = true; if (!$('#psDashboardShortcut')) { const button = document.createElement('button'); button.id = 'psDashboardShortcut'; button.className = 'ps-dashboard-shortcut ps-tip'; button.dataset.psTip = 'Yayıncı Dashboard'; button.textContent = 'Dashboard'; button.onclick = event => { event.preventDefault(); sessionStorage.setItem('ps-second-dashboard', '1'); $('#psSecondHome')?.setAttribute('hidden', ''); $('#authOverlay')?.setAttribute('hidden', ''); const app=$('.app');if(app){app.hidden=false;app.style.removeProperty('display')} }; actions.append(button); } const connection = $('#connectionBtn'); if (connection) { connection.classList.add('ps-tip'); connection.dataset.psTip = 'Bağlantı durumu'; if (connection.dataset.psIcon !== '1') { connection.dataset.psIcon = '1'; connection.innerHTML = '<span class="ps-connection-symbol">⌁</span>'; } } }
  ...
  window.addEventListener('unhandledrejection', event => {
    /* Arka plandaki isteğe bağlı yenilemeler (bildirim, cihaz ve bağlantı
       kontrolleri) ağ değişiminde reddedilebilir. Bu hatalar ilgili yüzeylerde
       zaten ele alınıyor; her reddedilen Promise için kullanıcıya genel bir
       hata bildirimi göstermek hem yanıltıcıydı hem de ekranı durmadan
       dolduruyordu. Gerçek kullanıcı işlemleri kendi form hata alanlarını
       kullanmaya devam eder. */
    if (event.reason?.message) console.warn('[Play Streamers] Background task failed:', event.reason.message);
  });
  refresh();
})();
...
(() => {
  ...
  function decorateSecond() { const home = $('#psSecondHome'); if (!home || home.hidden || !signed()) return; $('#psSecondAccountButton', home)?.remove(); const firstCard = $('.ps-second-grid .ps-second-card', home); if (firstCard && !$('#psSecondNotesArrow', firstCard)) { const title = $('h2', firstCard); if (title) { const row = document.createElement('div'); row.className = 'ps11-notes-head'; title.before(row); row.append(title); const arrow = document.createElement('button'); arrow.id = 'psSecondNotesArrow'; arrow.className = 'ps11-note-arrow'; arrow.type = 'button'; arrow.title = 'Güncelleme notlarını aç'; arrow.setAttribute('aria-label', 'Güncelleme notlarını aç'); arrow.textContent = '→'; arrow.onclick = showNotes; row.append(arrow); } }
    ...
    if (!$('#psSecondFooter', home)) { const footer = document.createElement('footer'); footer.id = 'psSecondFooter'; footer.className = 'ps11-footer'; footer.innerHTML = '<span>Play Streamers · Yayıncıların kontrol merkezi</span><a href="https://swcreate.com" target="_blank" rel="noopener noreferrer">Developed by SW CREATE</a>'; $('.ps-second-main', home)?.append(footer); }
...
(() => {
  ...
  const footerHtml = '<span>Güvenli bağlantılar · Kişisel panel · Ücretsiz başlangıç</span><span>Developed by <a href="https://swcreate.com" target="_blank" rel="noopener noreferrer"><strong>SW CREATE</strong></a></span>';
  const latestItems = [
...
(() => {
  ...
  function ensureSwitlyLink() { $$('a[href="https://swcreate.com"]').forEach(link => { link.target = '_blank'; link.rel = 'noopener noreferrer'; link.title = 'SW CREATE sitesini yeni sekmede aç'; }); }
...
(() => {
  ...
  function updatePublicPage() {
    ...
    const product = $('.product-carousel', overlay); if (product && !$('#ps15RichPanel', product)) { const panel = document.createElement('section'); panel.id = 'ps15RichPanel'; panel.className = 'ps15-panel-grid'; panel.innerHTML = '<article class="ps15-panel-card"><b>YAYINCI ARAÇLARI</b><div class="ps15-event-line"><i>İ</i><span>İçerik ve yayın planı</span><time>yerel</time></div><div class="ps15-event-line"><i>A</i><span>Analiz ve kanal hafızası</span><time>kanıtlı</time></div><div class="ps15-event-line"><i>M</i><span>Marka ve topluluk araçları</span><time>yerel</time></div></article><article class="ps15-panel-card"><b>DOĞRULANMIŞ VERİ İLKESİ</b><div class="ps15-mini-progress"><div><small>Hesap bağlı değilse</small><span><i style="width:18%"></i></span><small>—</small></div><div><small>Yayın kapalıysa</small><span><i style="width:18%"></i></span><small>—</small></div><div><small>Kaynak doğrulanmadıysa</small><span><i style="width:18%"></i></span><small>—</small></div></div></article>'; const controls = $('.carousel-controls', product); if (controls) product.insertBefore(panel, controls); else product.append(panel); }
  }
...
(() => {
  ...
  function enrichPanelPages() { const host = $('.product-carousel', $('#authOverlay') || document); if (!host) return; const extras = { '0': '<div><b>İÇERİK</b>Akış ve teleprompter</div><div class="purple"><b>TOPLULUK</b>Görev · bingo · hikâye</div><div class="gold"><b>MARKA</b>Görsel · rozet · medya kiti</div>', '1': '<div><b>VERİ İLKESİ</b>Doğrulanmadan gösterilmez</div><div class="purple"><b>AI AÇIKLAMASI</b>Kaynak veriden sonra</div><div class="gold"><b>GİZLİLİK</b>Hesaba özel görünüm</div>', '2': '<div><b>PLANLAR</b>Free · Pro · Product Pro</div><div class="purple"><b>VERİ AKIŞI</b>Kick · Play Connect</div><div class="gold"><b>YEREL KASA</b>Windows koruması</div>' }; $$('.product-page', host).forEach(page => { if ($('.ps16-page-extra', page)) return; const extra = document.createElement('section'); extra.className = 'ps16-page-extra'; extra.innerHTML = extras[page.dataset.page] || ''; page.append(extra); }); }
...
(() => {
  ...
  const esc = value => { const node = document.createElement('span'); node.textContent = String(value || ''); return node.innerHTML; };
  const summary = [
    'Kick yayınları site ve uygulama kapalıyken de sunucuda otomatik ölçülür.',
    'Ortalama ve tepe izleyici ile doğrulanmış etkileşimler yayın kapanınca özetlenir.',
    'Windows 10 ve 11 aynı 0.14.0 uygulama paketini kullanır.',
    'Play Connect Chrome ve Firefox paketleri 1.11.0 sürümünde eşitlendi.',
    'Studio ve yerel kayıt/yayın motoru rafa kaldırılmış durumda kalır.'
  ];
  const cardInfo = {
  ...
  function updatePublicRelease() { const overlay = $('#authOverlay'); if (!overlay) return; const update = $('.landing-update-card', overlay); if (update && update.dataset.ps17 !== '1') { const title = $('h2', update), intro = $('p', update), list = $('ul', update); if (title) title.textContent = '0.14.0 · Birleşik Premium Tasarım'; if (intro) intro.textContent = 'Masaüstü ve Play Connect, sitenin keskin siyah/beyaz tasarım dilinde birleşti; sunucu ölçümü arka planda sürer.'; if (list) list.innerHTML = summary.slice(0, 5).map(item => `<li>${esc(item)}</li>`).join(''); update.dataset.ps17 = '1'; }
...
(() => {
  ...
  function refreshStatus() { const button = $('#ps17SystemStatus'); if (!button) return; const local = evaluateLocal(); if (local) { applyStatus(button, local); return; } applyStatus(button, stateMessage('yellow', 'Sistem kontrol ediliyor', ['Cloudflare API erişimi ve temel servis durumu denetleniyor.'])); fetch(`${API}/health`, { cache: 'no-store' }).then(response => response.json().then(data => ({ response, data }))).then(({ response, data }) => { if (!response.ok || !data?.ok) throw new Error('Sağlık denetimi başarısız.'); applyStatus(button, stateMessage('green', 'Sistem normal', ['Ana sayfa bileşenleri yüklendi.', 'Cloudflare API erişilebilir görünüyor.', 'Giriş, kayıt ve bağlantı alanları kullanıma hazır.'])); }).catch(() => applyStatus(button, stateMessage('red', 'Sistem sorunu', ['Cloudflare API’ye ulaşılamıyor.', 'Giriş, kayıt ve canlı veri işlevleri şu anda kullanılamaz.']))); }
  ...
  function renameFooter() { $$('a[href="https://swcreate.com"]').forEach(link => { const strong = $('strong', link); if (strong) strong.textContent = 'SW CREATE'; else if (/developed by|switly/i.test(link.textContent || '')) link.textContent = 'SW CREATE'; link.title = 'SW CREATE sitesini aç'; }); }
...
(() => {
  ...
  function applyFooterBranding() {
    $$('.landing-footer').forEach(footer => {
      const label = footer.querySelector('strong, a[href="https://swcreate.com"]');
      if (!label) return;
...
(() => {
  ...
  function decorateFooter() { const foot = $('.app > .foot'); if (!foot) return; foot.className = 'foot ps13-footer'; foot.innerHTML = '<span>Güvenli bağlantılar · Kişisel panel · Ücretsiz başlangıç</span><span>Developed by <a href="https://swcreate.com" target="_blank" rel="noopener noreferrer">SW CREATE</a></span>'; }
...
(()=>{
  ...
  function release14(){const home=$('#psSecondHome');if(home){const h=$('.ps12-notes-heading h2',home);if(h)h.textContent='0.14.0 · Birleşik Premium Tasarım';const list=$('.ps12-notes-heading')?.parentElement?.querySelector('ul');if(list)list.innerHTML='<li>Masaüstü ve Play Connect sitenin keskin siyah/beyaz tasarım dilinde birleşti.</li><li>Kick yayınları site ve uygulama kapalıyken de sunucuda otomatik ölçülür.</li><li>Windows 10 ve 11 aynı konsolsuz uygulama paketini kullanır.</li><li>Studio ve yerel kayıt/yayın motoru rafa kaldırılmış durumda kalır.</li>'}const publicTitle=$('.landing-update-card h2');if(publicTitle)publicTitle.textContent='0.14.0 · Birleşik Premium Tasarım'}
  ...
  /* `restore()` ilk açılışta zaten çalışır. pageshow üzerinde tekrar çağırmak,
     ikinci ana sayfayı aynı oturumda yeniden kurup göz kırpmasına yol açıyordu. */
  addEventListener('pageshow',()=>{loader(false);repair()});
...
(() => {
  ...
  const langs = {
    tr:{flag:'🇹🇷',name:'Türkçe'}, en:{flag:'EN',name:'English'}, de:{flag:'🇩🇪',name:'Deutsch'}, es:{flag:'🇪🇸',name:'Español'},
  ...
  function translate(lang=currentLang()){
    const dict=words[lang]||words.tr; document.documentElement.lang=lang; document.documentElement.dir='ltr';
...
(() => {
  ...
  function openDashboardPs20(event){event?.preventDefault();event?.stopPropagation();closeFloats();loader(true);sessionStorage.setItem('ps-second-dashboard','1');const home=$('#psSecondHome'),app=$('.app'),auth=$('#authOverlay');if(home)home.hidden=true;if(auth)auth.hidden=true;if(app){app.hidden=false;app.style.removeProperty('display')}setTimeout(()=>{const panel=$('#panelView'),stats=$('#statsView');if(panel)panel.hidden=false;if(stats)stats.hidden=true;$$('.workspace-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.view==='panel'));loader(false)},290)}
  ...
  function showDashboard(event){event?.preventDefault();event?.stopPropagation();closeFloats();loader(true);sessionStorage.setItem('ps-second-dashboard','1');const home=$('#psSecondHome'),app=$('.app'),auth=$('#authOverlay');if(home)home.hidden=true;if(auth)auth.hidden=true;if(app){app.hidden=false;app.style.removeProperty('display')}setTimeout(()=>loader(false),260)}
  function render(withLoader=false){
    if(!active()||sessionStorage.getItem('ps-second-dashboard')==='1')return false;
    const existing=$('#psSecondHome');
    if(existing?.dataset.ps20Design==='1'&&existing.firstElementChild){
      existing.hidden=false;
      $('#authOverlay')?.setAttribute('hidden','');
      const currentApp=$('.app');
      if(currentApp){currentApp.hidden=true;currentApp.style.display='none'}
      return true;
    }
    const s=settings(),user=s.user||{},home=existing||document.createElement('section');
...
(() => {
  ...
  const googleSocialMark='<svg class="ps48-google-mark" viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M21.6 12.2c0-.7-.1-1.4-.2-2.1H12v4h5.4a4.6 4.6 0 0 1-2 3v2.6h3.2c1.9-1.8 3-4.3 3-7.5Z"/><path fill="#34A853" d="M12 22c2.7 0 5-.9 6.6-2.3l-3.2-2.6c-.9.6-2 1-3.4 1-2.6 0-4.8-1.8-5.6-4.1H3.1v2.7A10 10 0 0 0 12 22Z"/><path fill="#FBBC05" d="M6.4 14a6 6 0 0 1 0-4V7.3H3.1a10 10 0 0 0 0 9.4L6.4 14Z"/><path fill="#EA4335" d="M12 5.9c1.5 0 2.8.5 3.8 1.5l2.9-2.8A9.7 9.7 0 0 0 3.1 7.3L6.4 10c.8-2.3 3-4.1 5.6-4.1Z"/></svg>';
  function socialButton(provider){const node=document.createElement('button');node.type='button';node.className=`ps27-social ps27-social-${provider}${provider==='google'?' ps51-provider-button':''}`;node.dataset.ps27Social=provider;if(provider==='google')node.dataset.ps48Provider='google';node.setAttribute('aria-label',provider==='google'?'Google ile devam et':'Kick ile devam et');node.title=node.getAttribute('aria-label');node.innerHTML=provider==='google'?googleSocialMark:'K';return node}
  function decorateSocialButtons(){$$('form.auth-form,#landingAuthForm,#standaloneAuthForm').forEach(form=>{let row=$('.ps27-social-row,.ps55-provider-pair',form);if(!row){row=document.createElement('div');row.className='ps27-social-row';const existing=$('#modalGoogle,.google-button',form);if(existing){existing.classList.add('ps27-social','ps27-social-google','ps51-provider-button');existing.dataset.ps27Social='google';existing.dataset.ps48Provider='google';existing.setAttribute('aria-label','Google ile devam et');existing.title='Google ile devam et';existing.innerHTML=googleSocialMark;row.append(existing)}else row.append(socialButton('google'));const divider=$('.auth-divider',form);(divider||form.lastElementChild).after(row)}})}
...
(() => {
  ...
  /* Global arka plan hataları kullanıcı işlemi hatası değildir. Bildirim,
     bağlantı ve metrik yenilemeleri kısa bir ağ kesintisinde reddedildiğinde
     sürekli "beklenmeyen hata" göstermek yerine ilgili bileşenin bir sonraki
     kontrollü yenilemesini bekle. Form ve düğme işlemleri kendi görünür hata
     alanlarını kullanır. */
  window.addEventListener('error',event=>{if(event?.error)console.error('[Play Streamers] Runtime error:',event.error)});
  ...
  function loadThen(action){
    ...
    return;
    /* Eski uzun geçiş yükleyicisi davranış geriye dönüşü için kaynakta kalır,
       ancak normal sayfa geçişlerinde artık boş ekran oluşturmaz. */
  ...
  function connectKick(node){
    ...
    if(typeof window.ps67StartKickConnection==='function'){window.ps67StartKickConnection();return}
    location.assign(API+'/auth/kick/login');
  }
  ...
  function showDashboard(){
    closeAllPanels();sessionStorage.setItem('ps-second-dashboard','1');
...
(() => {
  ...
  // Canlı Kick olayları yalnızca üye yüzeyi görünürken sorgulanır.
  // `syncing` kilidi yavaş bir isteğin üstüne ikinci isteği bindirmez.
  const POLL_MS = 3_000;
  ...
  function saveSeen(ids) {
    localStorage.setItem(SEEN_KEY, JSON.stringify(ids.slice(-5000)));
  }
  ...
  function memberSurfaceIsActive() {
    ...
    const dashboardVisible = Boolean(dashboard && !dashboard.hidden && window.getComputedStyle(dashboard).display !== 'none');
    const homeVisible = Boolean(memberHome && !memberHome.hidden && window.getComputedStyle(memberHome).display !== 'none');
    return dashboardVisible || homeVisible;
  }
  async function syncKickEvents() {
    ...
    if (document.visibilityState !== 'visible' || navigator.onLine === false || !memberSurfaceIsActive()) return;
    if (syncing || !window.PlayStreamers?.addEvent) return;
    const state = readJson(STORE_KEY, null);
    const token = state?.settings?.kickSession || state?.settings?.userSession;
    if (!token) return;
    syncing = true;
    try {
      ...
      const needsHistory = accountChanged || seenIds().length === 0;
      if (accountChanged) {
        localStorage.removeItem(SEEN_KEY);
        localStorage.setItem(ACTIVE_BROADCASTER_KEY, broadcasterId);
      }
      if (needsHistory) {
        const historyResponse = await window.fetch(`${API}/api/kick/events?history=1`, {
          headers: { Authorization: `Bearer ${token}` },
          cache: 'no-store',
        });
        if (historyResponse.ok) data = await historyResponse.json();
      }
      const known = new Set(seenIds());
      const streamStartedAt = accountChanged ? 0 : Number(localStorage.getItem(STREAM_START_KEY) || 0);
      // Each destination applies its own reset boundary in addEvent.
      const resetBoundary = streamStartedAt;
      const additions = (Array.isArray(data.events) ? data.events : [])
        .filter((event) => !known.has(event.id))
        .filter((event) => !resetBoundary || eventTime(event.occurredAt || event.receivedAt) >= resetBoundary)
        .reverse();
  ...
  window.psSyncKickEvents = syncKickEvents;
  installBroadcastGuard();
  window.setTimeout(syncKickEvents, 250);
  window.setInterval(syncKickEvents, POLL_MS);
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') window.setTimeout(syncKickEvents, 100);
  });
  window.addEventListener('online', () => window.setTimeout(syncKickEvents, 100));
})();
...
(() => {
  ...
  function providerLogo(providerId) {
    const safeId = String(providerId || '').toLowerCase().replace(/[^a-z0-9_-]/g, '');
  ...
  async function syncDonateEvents() {
    ...
    try {
      ...
      for (let page = 0; page < 4; page += 1) {
        ...
        for (const event of events) {
          ...
          if (key && !seen.has(key)) {
            window.PlayStreamers.addEvent({
              ...
              sourceLogo: providerLogo(event.providerId),
        ...
        /* D1 olayları aynı milisaniyede yazılabilir. Son milisaniyeyi bir kez
           daha istemek veri kaybını önler; `seen` kümesi tekrarları eler. */
        localStorage.setItem(keys.cursor, String(Math.max(0, Math.max(cursor, Number(result.nextAfter || 0)) - 1)));
  ...
  window.setTimeout(syncDonateEvents, 200);
  window.setInterval(syncDonateEvents, POLL_MS);
  window.psSyncDonateEvents = syncDonateEvents;
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') window.setTimeout(syncDonateEvents, 100);
  });
  window.addEventListener('focus', () => window.setTimeout(syncDonateEvents, 100));
  window.addEventListener('online', () => window.setTimeout(syncDonateEvents, 100));
})();
...
(() => {
  ...
  const rescueVisibleSurface = (attempt = 0) => {
    ...
    let saved = {};
    try { saved = JSON.parse(localStorage.getItem('play-streamers-v17-site') || '{}') || {}; } catch (_) { saved = {}; }
    const signedIn = Boolean(saved.settings?.userSession || saved.userSession);
    const wantsDashboard = signedIn && (sessionStorage.getItem('ps-second-dashboard') === '1' || document.documentElement.dataset.psDashboardRestore === '1');
    if (signedIn) {
      if (overlay) { overlay.hidden = true; overlay.style.setProperty('display', 'none', 'important'); }
      document.body.classList.remove('auth-locked', 'onboarding-locked');
      if (wantsDashboard && app) {
        if (home) { home.hidden = true; home.style.setProperty('display', 'none', 'important'); }
        app.hidden = false;
        ...
        if (app) { app.hidden = true; app.style.setProperty('display', 'none', 'important'); }
        home.hidden = false;
        home.style.removeProperty('display'); home.style.removeProperty('visibility'); home.style.removeProperty('opacity');
      } else if (app) {
        app.hidden = false; app.style.removeProperty('display');
      }
    } else {
      if (home) home.hidden = true;
      if (app) { app.hidden = true; app.style.setProperty('display', 'none', 'important'); }
      if (overlay) { overlay.hidden = false; overlay.removeAttribute('hidden'); overlay.style.removeProperty('display'); overlay.style.removeProperty('visibility'); overlay.style.removeProperty('opacity'); }
      document.body.classList.add('auth-locked');
      document.body.classList.remove('onboarding-locked');
    }
  ...
  window.addEventListener('pageshow', () => {
    const home = document.querySelector('#psSecondHome');
    const app = document.querySelector('.app');
    const overlay = document.querySelector('#authOverlay');
    const alreadyVisible = [home, app, overlay].some(node => node && !node.hidden && getComputedStyle(node).display !== 'none');
    if (!alreadyVisible) setTimeout(boot, 0);
    scheduleSurfaceRescue();
  });
...
(() => {
  ...
  function ps41LocaleMenu(button) {
    ...
    $$('[data-language]', menu).forEach(choice => choice.onclick = () => {
      ...
      else { document.documentElement.lang=choice.dataset.language;document.documentElement.classList.remove('ps-i18n-booting','ps-locale-switching');window.dispatchEvent(new CustomEvent('ps:locale-change',{detail:{language:choice.dataset.language,source:'user'}})); }
    });
  }
  ...
  function moveLanguageButton() {
    $$('.landing-actions,.ps30-actions,.landing-nav .actions').forEach(actions => {
      ...
      if (!globe) {
        ...
        // Açma/kapama davranışının tek sahibi en son arayüz katmanıdır.
        // Burada ikinci bir click dinleyicisi eklemek menünün iki kez
        // tetiklenmesine ve düğmenin titriyor gibi görünmesine neden oluyordu.
      }
      globe.classList.add('ps41-locale-button');
      ...
      const anchor = status || login;
      if (globe && anchor && globe.nextElementSibling !== anchor) anchor.before(globe);
    });
  }

  const escapeHtml = value => String(value ?? '').replace(/[&<>"']/g, char => ({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' })[char]);
  function entry(name, meta, message) {
    return `<li class="read"><div class="event-main"><div class="event-top"><span class="name">${escapeHtml(name)}</span><span class="meta">${escapeHtml(meta)}</span></div><div class="message">${escapeHtml(message)}</div></div></li>`;
  }
