(() => {
  ...
  const money=(n,c='')=>new Intl.NumberFormat(interfaceLocale(),{style:'currency',currency:String(c||localeCurrency()).toUpperCase(),maximumFractionDigits:2}).format(Number(n||0)); const donateDt=t=>new Intl.DateTimeFormat(interfaceLocale(),{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date(t||Date.now())); const id=()=>Math.random().toString(36).slice(2)+Date.now().toString(36); const hue=s=>[...String(s)].reduce((a,c)=>(a*31+c.charCodeAt(0))%360,0); const kickProfileUrl=()=>state.settings.kickAccount?.username?`https://kick.com/${encodeURIComponent(state.settings.kickAccount.username)}`:'';
  ...
  window.addEventListener('ps:locale-change',()=>{window.requestAnimationFrame(()=>render())});
