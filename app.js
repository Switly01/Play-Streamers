(() => {
  ...
  function identityAuthorizePath(){const requestState=Array.from(crypto.getRandomValues(new Uint8Array(24)),byte=>byte.toString(16).padStart(2,'0')).join('');sessionStorage.setItem('ps-sw-identity-state',requestState);const path=new URL('/api/auth/product/authorize',SW_IDENTITY_ORIGIN);path.searchParams.set('client_id','play-streamers');path.searchParams.set('redirect_uri',SW_IDENTITY_REDIRECT);path.searchParams.set('state',requestState);return path}
  ...
  async function submitSwIdentityCredentials(form,isLogin,startedAt){
    ...
    try{
      const payload=isLogin
        ?{identity:form.elements.identity.value.trim(),password:form.elements.password.value,remember:form.elements.remember.checked,startedAt,website:''}
        :{username:form.elements.username.value.trim(),password:form.elements.password.value,passwordRepeat:form.elements.passwordRepeat.value,birthDate:form.elements.birthDate.value,remember:form.elements.remember.checked,startedAt,website:''};
      ...
      if(response.status===202&&data.twoFactorRequired){
        form.dataset.challengeId=data.challengeId;
        form.innerHTML=`<span class="eyebrow">İKİ AŞAMALI DOĞRULAMA</span><h3>Güvenlik kodunu gir</h3><p>Authenticator uygulamandaki 6 haneli kodu veya kurtarma kodunu kullan.</p><label class="auth-field">Doğrulama kodu<input name="code" autocomplete="one-time-code" inputmode="numeric" minlength="6" maxlength="9" required placeholder="000000"></label><p class="auth-error ps-identity-form-error" aria-live="polite"></p><button class="auth-submit ps-identity-form-submit" type="submit">Doğrula ve devam et</button>`;
        form.onsubmit=async event=>{event.preventDefault();const verifyError=$('.ps-identity-form-error',form),verifyButton=$('.ps-identity-form-submit',form);verifyButton.disabled=true;verifyButton.textContent='Doğrulanıyor…';try{const verification=await fetch(`${SW_IDENTITY_ORIGIN}/api/auth/two-factor/verify`,{method:'POST',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({challengeId:form.dataset.challengeId,code:form.elements.code.value})});const result=await verification.json().catch(()=>({}));if(!verification.ok)throw new Error(result.error||'Kod doğrulanamadı.');location.assign(identityAuthorizePath().toString())}catch(verifyFailure){verifyError.textContent=verifyFailure.message||'Kod doğrulanamadı.';verifyButton.disabled=false;verifyButton.textContent='Doğrula ve devam et'}};
        form.elements.code?.focus();return;
      }
      ...
      location.assign(identityAuthorizePath().toString());
  ...
  function showLandingAuthV101(mode){
    ...
    const kickIcon='<svg viewBox="0 0 24 28" aria-hidden="true"><path fill-rule="evenodd" d="M1 1H9.7V7.15H12.6V4.08H15.5V1H24V10.23H21.1V13.31H18.2V16.38H21.1V19.46H24V27H15.5V24H12.6V20.92H9.7V27H1V1Z"/></svg>';
    ...
    $('[data-provider="google"]',layer).onclick=()=>startSwIdentityLogin('google');$('[data-provider="kick"]',layer).onclick=()=>startSwIdentityLogin('kick');$('[data-provider="sw"]',layer)?.addEventListener('click',()=>startSwIdentityLogin());
  }
