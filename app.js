(() => {
  ...
  installUserAuthControls = function installCreatorUserAuthControls() {
    ...
    const updateHeading = $('h2', update); if (updateHeading) updateHeading.textContent = '0.12.0 · Sade ve Sessiz Masaüstü';
  };
...
(() => {
  ...
  const esc = value => { const node = document.createElement('span'); node.textContent = String(value || ''); return node.innerHTML; };
  const summary = [
    'Studio ve yerel kayıt/yayın motoru masaüstü uygulamasından rafa kaldırıldı.',
    'Genel kayıt, yayın ve replay kısayolları kaldırıldı.',
    'Uygulama 45 içerik, analiz, topluluk, marka, gelir, kasa ve ayar aracına odaklandı.',
    'Windows üretim sürümü CMD penceresi açmadan bağımsız çalışacak biçimde düzeltildi.',
    'SW Identity, güvenli oturum, doğrulanmış Kick ve Play Connect veri akışları korundu.'
  ];
  const cardInfo = {
  ...
  function updatePublicRelease() { const overlay = $('#authOverlay'); if (!overlay) return; const update = $('.landing-update-card', overlay); if (update && update.dataset.ps17 !== '1') { const title = $('h2', update), intro = $('p', update), list = $('ul', update); if (title) title.textContent = '0.12.0 · Sade ve Sessiz Masaüstü'; if (intro) intro.textContent = 'Studio rafa kaldırıldı; uygulama artık yalnız yayıncı araçlarına odaklanıyor ve CMD penceresi açmadan çalışıyor.'; if (list) list.innerHTML = summary.slice(0, 5).map(item => `<li>${esc(item)}</li>`).join(''); update.dataset.ps17 = '1'; }
...
(()=>{
  ...
  function release14(){const home=$('#psSecondHome');if(home){const h=$('.ps12-notes-heading h2',home);if(h)h.textContent='0.12.0 · Sade ve Sessiz Masaüstü';const list=$('.ps12-notes-heading')?.parentElement?.querySelector('ul');if(list)list.innerHTML='<li>Studio ve kayıt/yayın motoru masaüstü uygulamasından rafa kaldırıldı.</li><li>Uygulama içerik, analiz, topluluk, marka, gelir ve kasa araçlarına odaklandı.</li><li>Genel kayıt/yayın kısayolları kaldırıldı.</li><li>Windows üretim sürümü artık CMD penceresi açmadan çalışır.</li>'}const publicTitle=$('.landing-update-card h2');if(publicTitle)publicTitle.textContent='0.12.0 · Sade ve Sessiz Masaüstü'}
