(() => {
  ...
  const FX_CACHE_KEY='ps116-exchange-rates'; const DONATE_VISIBLE_AGE_MS=24*60*60*1000; let exchangeRates=null; try{const cached=JSON.parse(localStorage.getItem(FX_CACHE_KEY)||'null');if(cached?.rates&&Date.now()-Number(cached.savedAt||0)<1800000)exchangeRates=cached}catch{}
  ...
  function markRead(kind,item,row){if(!item||item.read)return;item.read=true;item.readAt=Date.now();save();if(row){row.classList.remove('new');row.classList.add('read')}scheduleRemove(kind,item)}
  ...
  function scheduleRemove(kind,item){const k=kind+item.id;if(removalTimers.has(k))return;removalTimers.add(k);const wait=Math.max(0,60000-(Date.now()-(item.readAt||Date.now())));setTimeout(()=>{if(kind==='subs')delete state.events.subs[item.name.toLowerCase()];else state.events[kind]=state.events[kind].filter(x=>x.id!==item.id);save();removalTimers.delete(k);removeRenderedEvent(kind,item)},wait)}
  function openEvent(item,kind){if(!item)return;const donate=kind==='donations';const sourceUrl=String(item.sourceUrl||'').trim();const hasSource=/^https?:\/\//i.test(sourceUrl);const sourceAction=!donate&&hasSource?`<a href="${esc(sourceUrl)}" target="_blank" rel="noopener noreferrer">Kick profilini aç ↗</a>`:(!donate?'<span class="event-source-unavailable">Kaynak bağlantısı yok</span>':'');const sourceBrand=donate?`<div class="event-source-brand">${item.sourceLogo?`<img src="${esc(item.sourceLogo)}" alt="${esc(item.sourceName||'Donate')} logosu">`:''}<span>${esc(item.sourceName||'Donate')}</span></div>`:'';const eventMeta=donate?`${money(item.amount,item.currency)} · ${donateDt(item.at)}`:(item.meta||dt(item.at));const donateDetails=donate?`<dl class="event-detail-grid"><div><dt>Gönderen</dt><dd>${esc(item.name||'Anonim')}</dd></div><div><dt>Tutar</dt><dd>${esc(money(item.amount,item.currency))}</dd></div><div><dt>Tarih ve saat</dt><dd>${esc(donateDt(item.at))}</dd></div><div><dt>Platform</dt><dd>${esc(item.sourceName||'Donate')}</dd></div></dl><section class="event-detail-message"><span>Mesaj</span><p>${eventMessage(item.message||'Mesaj bırakılmadı.')}</p></section>`:'';$('#modal').hidden=false;$('#modal').innerHTML=`<section class="dialog${donate?' event-detail-dialog':''}"><button class="close">×</button><span class="eyebrow">${donate?'DONATE AYRINTISI':'OLAY AYRINTISI'}</span>${sourceBrand}<h2>${esc(item.name)}</h2>${donate?donateDetails:`<div class="event-message">${eventMessage(item.message)}</div><div class="event-foot"><span>${esc(eventMeta)}</span>${sourceAction}</div>`}</section>`;$('#modal .close').onclick=closeModal;$('#modal').onclick=e=>{if(e.target===$('#modal'))closeModal}}
  ...
  function convertedDonationStats(){const result={month:{},all:{}},target=localeCurrency();state.events.donations.forEach(item=>{const converted=convertMoney(item.amount,item.currency||'TRY',target);if(converted===null)return;const key=String(item.name||'Destekçi').toLowerCase(),at=Number(item.at||Date.now()),monthKey=new Date(at).toISOString().slice(0,7);const add=bucket=>{bucket[key]??={name:item.name||'Destekçi',total:0,at,currency:target};bucket[key].total+=converted;bucket[key].at=Math.max(bucket[key].at,at)};add(result.all);result.month[monthKey]??={};add(result.month[monthKey])});return result}
  ...
  window.PlayStreamers={addEvent(event){const at=Number(event.at)||Date.now(),type=event.type;state.events.followers??=[];state.totals.followers??=state.events.followers.length;if(type==='follower'){const row={id:event.id||id(),name:event.name,message:event.message||`${event.name} takip etti.`,at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};state.events.followers.unshift(row);state.totals.followers+=1}if(type==='subscription'){const key=event.name.toLowerCase(),old=state.events.subs[key],months=Math.max(1,Number(event.months||1)),row={id:event.id||id(),name:event.name,months:Math.max(Number(old?.months||0),months),expiresAt:event.expiresAt||old?.expiresAt||'',message:event.message||`${event.name} abone oldu.`,at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};state.events.subs[key]=row;addStat('subs',event.name,row.months,at)}if(type==='kicks'){const row={id:event.id||id(),name:event.name,amount:+event.amount||0,message:event.message||'',at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};state.events.kicks.unshift(row);state.totals.kicks+=row.amount;addStat('kicks',row.name,row.amount,at)}if(type==='gift'){const row={id:event.id||id(),name:event.name,count:+event.count||1,message:event.message||'',at,sourceUrl:event.sourceUrl||kickProfileUrl(),read:false};state.events.gifts.unshift(row);state.totals.gifts+=row.count;addStat('gifts',row.name,row.count,at)}if(type==='donation'){const row={id:event.id||id(),name:event.name,amount:+event.amount||0,currency:event.currency||localeCurrency(),message:event.message||'',sourceIcon:event.sourceIcon||new Intl.NumberFormat(interfaceLocale(),{style:'currency',currency:localeCurrency(),currencyDisplay:'narrowSymbol'}).formatToParts(0).find(part=>part.type==='currency')?.value||localeCurrency(),sourceName:event.sourceName||'Donate',sourceLogo:event.sourceLogo||'',providerId:event.providerId||'',sourceUrl:event.sourceUrl||'',at,read:false};state.events.donations.unshift(row);state.totals.donations+=row.amount;addStat('donations',row.name,row.amount,at)}save();render()},resetPanel(){state.events={followers:[],subs:{},kicks:[],gifts:[],donations:[]};state.totals={followers:0,kicks:0,donations:0,gifts:0};state.settings.panelResetAt=Date.now();save();render()},resetStats(){state.stats={subs:{},joined:{},gifts:{month:{},all:{}},kicks:{month:{},all:{}},donations:{month:{},all:{}}};state.settings.statsResetAt=Date.now();save();render()},renderDashboard(){render()},startBroadcast(){state.events={followers:[],subs:{},kicks:[],gifts:[],donations:[]};state.totals={followers:0,kicks:0,donations:0,gifts:0};state.stats={subs:{},joined:{},gifts:{month:{},all:{}},kicks:{month:{},all:{}},donations:{month:{},all:{}}};save();render()}};
...
(() => {
  ...
  async function syncKickEvents() {
    ...
    try {
      ...
      const resetBoundary = Math.max(Number(state.settings?.panelResetAt || 0), Number(state.settings?.statsResetAt || 0), streamStartedAt);
