(() => {
  ...
  installUserAuthControls = function installCreatorUserAuthControls() {
    ...
    const updateHeading = $('h2', update); if (updateHeading) updateHeading.textContent = '0.13.0 · Otomatik Sunucu Analizi';
  };
...
(() => {
  ...
  const summary = [
    ...
    'Windows 10 ve 11 aynı 0.13.0 uygulama paketini kullanır.',
  ...
  function updatePublicRelease() { const overlay = $('#authOverlay'); if (!overlay) return; const update = $('.landing-update-card', overlay); if (update && update.dataset.ps17 !== '1') { const title = $('h2', update), intro = $('p', update), list = $('ul', update); if (title) title.textContent = '0.13.0 · Otomatik Sunucu Analizi'; if (intro) intro.textContent = 'Kick yayınları site ve uygulama kapalıyken de sunucuda ölçülür; Windows 10/11 aynı sade paketi kullanır.'; if (list) list.innerHTML = summary.slice(0, 5).map(item => `<li>${esc(item)}</li>`).join(''); update.dataset.ps17 = '1'; }
...
(()=>{
  ...
  function release14(){const home=$('#psSecondHome');if(home){const h=$('.ps12-notes-heading h2',home);if(h)h.textContent='0.13.0 · Otomatik Sunucu Analizi';const list=$('.ps12-notes-heading')?.parentElement?.querySelector('ul');if(list)list.innerHTML='<li>Kick yayınları site ve uygulama kapalıyken de sunucuda otomatik ölçülür.</li><li>Ortalama ve tepe izleyici, takip, abonelik, bağış ve etkileşim özeti sunucuda tamamlanır.</li><li>Windows 10 ve 11 aynı konsolsuz uygulama paketini kullanır.</li><li>Studio ve yerel kayıt/yayın motoru rafa kaldırılmış durumda kalır.</li>'}const publicTitle=$('.landing-update-card h2');if(publicTitle)publicTitle.textContent='0.13.0 · Otomatik Sunucu Analizi'}
