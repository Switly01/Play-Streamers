(() => {
  ...
  installUserAuthControls = function installCreatorUserAuthControls() {
    ...
    const heading = $('h1', hero); if (heading) heading.textContent = 'Yayın aç. Kaydet. Büyümeyi tek yerden yönet.';
    ...
    const updateHeading = $('h2', update); if (updateHeading) updateHeading.textContent = '0.11.0 · Çoklu Yayın ve Tuval';
  };
    ...
    host.innerHTML = `<section class="product-window" aria-label="Play Streamers temsili masaüstü arayüz demosu">
      ...
      <div class="product-carousel">
        <section class="product-page" data-page="0">
          <div class="preview-hero"><div><span class="preview-kicker">STUDIO · PROGRAM GÖRÜNÜMÜ</span><h3>Sahne akışını tek ekranda yönet.</h3><p>Bu alan ürün görünümünü tanıtan bir arayüz demosudur; canlı hesap verisi içermez.</p><div class="preview-live"><i></i>ARAYÜZ DEMOSU</div></div><div class="preview-stat-stack"><div><b>32</b><span>kalıcı sahne</span></div><div><b>64</b><span>sıralanabilir ek kaynak</span></div></div></div>
          <div class="preview-columns"><article class="preview-panel"><span class="preview-kicker">SAHNE AKIŞI</span><h4>Program sırası</h4><div class="preview-event"><i>01</i><b>Açılış sahnesi</b><span>hazır</span></div><div class="preview-event"><i>02</i><b>Ana yayın</b><span>hazır</span></div><div class="preview-event"><i>03</i><b>Mola ekranı</b><span>hazır</span></div></article><article class="preview-panel"><span class="preview-kicker">SES ZİNCİRİ</span><h4>Filtre ön izlemesi</h4><div class="preview-bars"><i style="height:28%"></i><i></i><i></i><i></i><i></i><i></i></div></article></div>
        </section>
        ...
        <section class="product-page" data-page="2" hidden>
          ...
          <div class="preview-columns"><article class="preview-panel"><span class="preview-kicker">ÇALIŞMA ALANLARI</span><h4>54 hazır araç</h4><div class="preview-event"><i>İ</i><b>İçerik ve yayın planı</b><span>yerel</span></div><div class="preview-event"><i>T</i><b>Topluluk ve büyüme</b><span>doğrulanmış</span></div><div class="preview-event"><i>K</i><b>Marka ve yaratıcı kasa</b><span>güvenli</span></div></article><article class="preview-panel"><span class="preview-kicker">KONTROL</span><h4>Site sade, üretim uygulamada.</h4><p style="color:#aec0d4;font-size:12px;line-height:1.6">Sitede hesap, plan ve bağlantıları; uygulamada Studio ile bütün üretim sistemini yönet.</p></article></div>
        </section>
...
(() => {
  ...
  function updatePublicPage() {
    ...
    const product = $('.product-carousel', overlay); if (product && !$('#ps15RichPanel', product)) { const panel = document.createElement('section'); panel.id = 'ps15RichPanel'; panel.className = 'ps15-panel-grid'; panel.innerHTML = '<article class="ps15-panel-card"><b>STUDIO KONTROLLERİ</b><div class="ps15-event-line"><i>S</i><span>Sahne ve katman düzeni</span><time>yerel</time></div><div class="ps15-event-line"><i>G</i><span>Geçiş ve program görünümü</span><time>motor</time></div><div class="ps15-event-line"><i>S</i><span>Ses filtre zinciri</span><time>motor</time></div></article><article class="ps15-panel-card"><b>DOĞRULANMIŞ VERİ İLKESİ</b><div class="ps15-mini-progress"><div><small>Hesap bağlı değilse</small><span><i style="width:18%"></i></span><small>—</small></div><div><small>Yayın kapalıysa</small><span><i style="width:18%"></i></span><small>—</small></div><div><small>Kaynak doğrulanmadıysa</small><span><i style="width:18%"></i></span><small>—</small></div></div></article>'; const controls = $('.carousel-controls', product); if (controls) product.insertBefore(panel, controls); else product.append(panel); }
  }
...
(() => {
  ...
  function updatePublicRelease() { const overlay = $('#authOverlay'); if (!overlay) return; const update = $('.landing-update-card', overlay); if (update && update.dataset.ps17 !== '1') { const title = $('h2', update), intro = $('p', update), list = $('ul', update); if (title) title.textContent = '0.11.0 · Çoklu Yayın ve Tuval'; if (intro) intro.textContent = 'İki RTMPS çıkışı, tuvalde sürükleme, 32 sahne ve gerçek CPU telemetrisi.'; if (list) list.innerHTML = summary.slice(0, 5).map(item => `<li>${esc(item)}</li>`).join(''); update.dataset.ps17 = '1'; }
...
(()=>{
  ...
  function release14(){const home=$('#psSecondHome');if(home){const h=$('.ps12-notes-heading h2',home);if(h)h.textContent='0.11.0 · Çoklu Yayın ve Tuval';const list=$('.ps12-notes-heading')?.parentElement?.querySelector('ul');if(list)list.innerHTML='<li>Tek kodlama akışı iki güvenli RTMPS hedefine gönderilebilir.</li><li>Kaynaklar Önizleme tuvalinde sürüklenerek konumlandırılabilir.</li><li>32 sahne ve sahne başına 64 kaynak desteklenir.</li><li>Gerçek FFmpeg CPU kullanımı telemetride görünür.</li>'}const publicTitle=$('.landing-update-card h2');if(publicTitle)publicTitle.textContent='0.11.0 · Çoklu Yayın ve Tuval'}
