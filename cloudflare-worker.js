...
async function runScheduledPlayBotAudit(env) {
  ...
  const resources = [
    ...
    ["Fransızca dil paketi", "https://pstreamers.com/locales/fr.json?v=2026-08-29.4", "json"],
  ...
  for (const result of results) {
    ...
    if (result.type === "json") {
      ...
      const validPayload = result.label === "Windows güncelleme bildirimi"
        ...
        : localeCatalog
          ? Boolean(payload?.version === "2026-08-29.4" && payload?.sourceLanguage === "tr" && payload?.language && Object.keys(payload?.translations || {}).length >= 350)
          : Boolean(payload?.ok);
