(() => {
  ...
  function destroy() { clearInterval(countdownTimer); countdownTimer=null; root?.remove(); root = null; document.documentElement.classList.remove('ps-creator-page-open'); }
  ...
  async function render(slug, embeddedPayload = null) {
    ...
    try {
      ...
      countdownTimer=setInterval(()=>root?.querySelectorAll('.pscp-countdown[data-date]').forEach(item=>{item.textContent=countdown(item.dataset.date)}),1000);
