...
const UI_MESSAGE_TYPES = new Set([
  "SET_UI_LOCALE", "GET_STATE", "GET_PROVIDER_ALERT_URL", "PAIR_ACCOUNT",
  "DISCONNECT_ACCOUNT", "SAVE_PROVIDER", "TEST_PROVIDER", "POLL_NOW",
  "SEND_SUPPORT", "OPEN_PROVIDER_LOGIN", "OPEN_PROVIDER_LOGOUT"
]);

function isTrustedExtensionPage(sender, paths) {
  return sender?.id === chrome.runtime.id && paths.some(path =>
    String(sender?.url || "").startsWith(chrome.runtime.getURL(path))
  );
}

// Content scripts must not read or modify the credential-bearing local state.
...
// Chrome ayakta olduğu sürece yeni olayları olabilecek en kısa sürede işler.
...
async function existingManagedWindow(windowId) {
  if (!windowId || !chrome.windows?.get) return null;
  try {
...
async function openManagedProvider(provider, targetUrl) {
  ...
  if (browserWindow) {
    ...
    if (tab?.id) {
      ...
      tab = await chrome.tabs.create({ windowId: browserWindow.id, url: targetUrl, active: true });
    }
    await chrome.windows.update(browserWindow.id, { focused: true }).catch(() => {});
  } else {
    browserWindow = await chrome.windows.create({
      url: targetUrl,
      incognito: false,
      type: "popup",
      focused: true,
...
async function managedCookieStoreId(tabId) {
  if (!tabId || !chrome.cookies?.getAllCookieStores) return "";
...
async function closeLearningSurface(providerId) {
  ...
  if (tabId && chrome.tabs?.remove) await chrome.tabs.remove(tabId).catch(() => {});
  return publicState(mutation.state);
}
...
async function syncByNoGameCookieSession(force = false) {
  if (!chrome.cookies?.getAll) return { detected: false, changed: false };
  const now = Date.now();
  ...
  const mutation = await mutateState(state => {
    ...
    state.providers.bynogame = config;
    if (!hadSession && token) {
      activity(state, "success", "ByNoGame oturumu tarayıcıda doğrulandı; oturum bilgisi yalnız bu Chrome profilinde tutuluyor.", "bynogame");
...
async function ensureOffscreenDocument() {
  ...
  try {
    await chrome.offscreen.createDocument({
      ...
      justification: "Kullanıcının OBS alert bağlantısını görünür sekme açmadan çalıştırmak ve donate verisini yerel olarak ayrıştırmak."
    });
  } catch (error) {
...
async function syncAlertSources(stateInput = null) {
  ...
  if (!sources.length) {
    ...
    const contexts = chrome.runtime.getContexts
      ...
      : [];
    if (!contexts.length) return { ok: true, active: 0 };
  } else {
...
async function scanProvider(providerId, manual = false) {
  ...
  if (provider.integration === "session" && !hasDirectSessionFeed) {
    ...
    try {
      const pageResult = await chrome.tabs.sendMessage(config.managedTabId, { type: "PLAY_CONNECT_SCAN_NOW" });
      ...
      if (shouldRefreshManagedPage && chrome.tabs?.reload) {
        await mutateState(currentState => {
          currentState.providers[providerId].lastManagedRefreshAt = Date.now();
        });
        await chrome.tabs.reload(latest.managedTabId).catch(() => {});
      }
      return {
        ok: true,
        accepted,
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  refreshManagedBrowserStatus()
    .then(result => {
      if (!result.allowed && chrome.runtime.openOptionsPage) return chrome.runtime.openOptionsPage();
      return undefined;
    })
    .catch(() => {});
  scanAll().catch(() => {});
...
try {
  ...
  // Chrome sürümü extraHeaders seçeneğini sunmuyorsa temel başlık görünümüyle devam et.
  try {
    chrome.webRequest?.onBeforeSendHeaders?.addListener(
      rememberPageBearer,
      { urls: providerRequestPatterns() },
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  (async () => {
    await storageAccessReady;
    if (UI_MESSAGE_TYPES.has(message?.type) && !isTrustedExtensionPage(sender, ["options/", "popup/"])) {
      throw new Error("İşlem yalnız Play Connect ekranından yapılabilir.");
    }
    if (message?.type === "ALERT_FRAME_STATUS" && !isTrustedExtensionPage(sender, [OFFSCREEN_PATH])) {
      throw new Error("Geçersiz OBS durum kaynağı.");
    }
    switch (message?.type) {
      case "SET_UI_LOCALE": {
      ...
      case "GET_PROVIDER_ALERT_URL": {
        const optionsRoot = chrome.runtime.getURL("options/");
        if (sender?.id !== chrome.runtime.id || !String(sender?.url || "").startsWith(optionsRoot)) {
      ...
      case "PAGE_MONITOR_DISCOVERY": {
        ...
        if (mutation.result.shouldNavigate && sender.tab?.id) {
          await chrome.tabs.update(sender.tab.id, { url: mutation.result.monitorUrl, active: true }).catch(() => {});
        }
