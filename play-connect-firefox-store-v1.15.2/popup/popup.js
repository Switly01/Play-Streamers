...
async function openPanel(providerId = "") {
  if (providerId) await browser.storage.session?.set?.({ openProviderId: providerId }).catch(() => {});
  const query = providerId ? `?provider=${encodeURIComponent(providerId)}` : "";
  const url = browser.runtime.getURL(`options/options.html${query}`);
  try {
...
function render(state) {
  ...
  $("#pairText").textContent = paired
    ? `${connection.deviceName || "Firefox Eklentisi"} · Platform giriş bilgilerin bu cihazda kalır.`
