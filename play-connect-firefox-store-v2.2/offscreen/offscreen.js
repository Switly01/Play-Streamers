(() => {
  ...
  function reportAlertFrame(providerId, status, detail = "") {
    ...
    const directReporter = globalThis.__PLAY_CONNECT_REPORT_ALERT_FRAME__;
    if (typeof directReporter === "function") {
      Promise.resolve(directReporter(payload)).catch(() => {});
      return;
    }
  ...
  function dispatchOffscreenMessage(message) {
    ...
    if (message?.type === "SYNC_ALERT_SOURCES") {
      return syncAlertFrames(message.sources);
    }
    if (message?.type !== "PARSE_DONATE_HTML") return null;
    const html = String(message.html || "");
    if (!html || html.length > MAX_HTML_BYTES) throw new Error("Platform yanıtı boş veya çok büyük.");
    const parsedDocument = new DOMParser().parseFromString(html, "text/html");
    return {
      ok: true,
      loginRequired: loginRequired(parsedDocument, message.pageUrl),
      candidates: candidates(parsedDocument, message.selectors || {})
    };
  }

  // Firefox paketinde bu dosya kalıcı arka plan sayfasında çalışır. Modül
  // betiği, runtime mesajının aynı pencereye geri dönmesine ihtiyaç duymadan
  // ayrıştırıcıya ve görünmeyen OBS iframe'lerine doğrudan erişir.
  globalThis.__PLAY_CONNECT_OFFSCREEN__ = Object.freeze({
    dispatch: dispatchOffscreenMessage
  });

  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.target !== "offscreen") return false;
    if (sender?.id !== browser.runtime.id || sender?.tab || (sender?.url && !String(sender.url).startsWith(browser.runtime.getURL("")))) return false;
    try {
      const result = dispatchOffscreenMessage(message);
