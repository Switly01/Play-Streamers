...
async function renderReleaseNotes() {
  ...
  const textNode = (tag, value) => {
    const node = document.createElement(tag);
    node.textContent = String(value ?? '');
    return node;
  };
  $('#updateNotesList').replaceChildren(...product.entries.map((note, index) => {
    const article = document.createElement('article');
    article.className = note.beta ? 'beta-release' : 'stable-release';
    const header = document.createElement('header');
    if (note.beta || note.version === '1.0') header.append(textNode('span', note.beta ? archive.ui.beta : archive.ui.fullRelease));
    header.append(textNode('b', note.version));
    if (index === 0) header.append(textNode('i', archive.ui.latest));
    const list = document.createElement('ul');
    list.append(...note.items.map(item => textNode('li', item)));
    article.append(header, textNode('h3', note.title), list);
    return article;
  }));
}
...
function renderConnection() {
  ...
  $("#serverText").textContent = paired
    ? `${connection.deviceName || "Firefox Eklentisi"} · ${responseText} · Sunucuda: ${Number(connection.lastServerEventCount || 0)} · Bekleyen: ${state.queueCount || 0}`
  ...
  $("#pairDescription").textContent = paired
    ? "Platform oturumları ve erişim anahtarları bu Firefox profilinde kalır. Sunucuya yalnızca normalleştirilmiş donate olayları gider."
...
function alertFields(provider) {
  ...
  return `<label class="wide">OBS / Alert Box bağlantısı
    ...
    <small>${config.hasAlertUrl ? "Bağlantı kilitli; değiştirmek için önce aşağıdaki OBS bağlantısını kaldır düğmesini kullan." : "Bu bağlantı bir şifre gibi gizlidir; yalnızca bu Firefox profilinde tutulur ve Play Streamers sunucusuna gönderilmez."}</small>
  </label>`;
}
  ...
  if (["tipeeestream-api", "donationalerts-api", "pally-api"].includes(provider.integration)) {
    return {
      ...
      fields: `
        <label class="wide">${esc(provider.apiTokenLabel)}
          <input name="apiToken" type="password" autocomplete="off" placeholder="Yeni anahtar gir veya kayıtlı anahtarı değiştirme">
          <small>Anahtar yalnızca bu Firefox profilinde saklanır. <a href="${esc(provider.apiHelpUrl)}" target="_blank" rel="noreferrer">API belgesi ↗</a></small>
        </label>`
    };
  }
  return {
...
function renderProvider() {
  ...
  const loginCopy = config.loginStatus === "observed"
    ? `Firefox oturumu algılandı · donate veri akışı otomatik bağlandı${config.lastPageAt ? ` · ${new Date(config.lastPageAt).toLocaleString(currentLocale())}` : ""}`
  ...
  const managedHealth = managedSession
    ? `<section class="managed-health ${backgroundActive ? "is-active" : (config.alertFrameStatus === "error" ? "is-error" : "is-ready")}">
        ...
        <small>${backgroundActive
          ? `${esc(config.alertRenderer || provider.name)} bağlantısı Firefox açıkken görünür sekme olmadan yeni donate olaylarını dinliyor.`
...
function bindProviderForm(provider) {
  ...
  form.addEventListener("submit", async event => {
    ...
    try {
      ...
      await loadActiveAlertUrl(provider.id);
      status.textContent = "Ayarlar bu Firefox profilinde güvenli biçimde kaydedildi.";
      renderNavigation();
  ...
  $("#clearToken")?.addEventListener("click", async event => {
    if (!confirm(translate("Bu platform için bu Firefox profilinde saklanan erişim anahtarı silinsin mi?"))) return;
    const button = event.currentTarget;
    button.disabled = true;
    try {
...
async function initialize() {
  try {
    ...
    browser.storage.session?.remove?.("openProviderId").catch(() => {});
    renderConnection();
    renderNavigation();
    renderProvider();
...
$('#updateNotesButton').addEventListener('click', () => void openReleaseNotes().catch(error => {
  $('#pairStatus').className = 'form-status error';
  $('#pairStatus').textContent = error.message;
}));
...
document.addEventListener('keydown', event => {
  if (event.key === 'Tab' && !$('#updateNotesModal').hidden) {
    event.preventDefault();
    $('#updateNotesClose').focus();
  }
