...
function isTrustedExtensionPage(sender, paths) {
  return sender?.id === browser.runtime.id && paths.some(path =>
    String(sender?.url || "").startsWith(browser.runtime.getURL(path))
  );
}

const storageAccessReady = browser.storage.local.setAccessLevel
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  (async () => {
    ...
    if (message?.type === "ALERT_FRAME_STATUS" && !isTrustedExtensionPage(sender, ["firefox/background.html"])) {
