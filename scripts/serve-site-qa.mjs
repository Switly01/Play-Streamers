...
<nav id="qaControls" style="position:fixed;bottom:8px;left:8px;z-index:70000;display:flex;gap:6px;background:#181818;padding:8px"><button data-qa="data">Hesap verileri</button><button data-qa="account">Güvenlik</button><button data-qa="profile">Profil</button><button data-qa="connections">Bağlantılar</button><button data-qa="products">Ürünler</button><button data-qa="donation">Bağış kartı</button><button data-qa="tools">Plan araçları</button><select id="qaLanguage" aria-label="Test dili"><option>tr</option><option>en</option><option>de</option><option>es</option><option>fr</option><option>ru</option><option>ar</option><option>ja</option></select><output id="qaSsbLanguage" data-no-translate></output></nav>
...
window.psExchangeRates={rates:{TRY:50,EUR:1,USD:1.1,RUB:90,SAR:4.1,JPY:160},rateDate:'2026-09-03',savedAt:Date.now()};
...
if(url.pathname==='/api/donate-webhooks/connections/test')document.getElementById('qaSsbLanguage').textContent='SSB: '+JSON.parse(options.body).language;
...
if(url.pathname==='/api/kick/session')data={connected:true,account:{id:'qa-kick',username:'test-channel',profilePicture:'https://qa.example.test/kick-avatar.png'}};
if(url.pathname==='/api/donate-bridge/devices')data={ok:true,devices:[],webhookConnections:[{id:'11111111-1111-4111-8111-111111111111',providerId:'kofi',providerName:'Ko-fi',eventCount:0}],providers:[{id:'bynogame',name:'ByNoGame',serverWebhook:false},{id:'itemsatis',name:'İtemSatış',serverWebhook:true},{id:'kofi',name:'Ko-fi',serverWebhook:true}]};
...
if(url.pathname.includes('/api/kick/events'))data={ok:true,broadcasterId:'qa-kick',insights:{...qaState.settings.kickInsights,activeFollowers:27,activeSubscribers:2,followersSource:'kick-api',subscribersSource:'kick-api',followedThisMonth:9,hourlyMetrics:[{hour:'06',followersCount:27},{hour:'07',followersCount:null}]},events:[]};
...
document.querySelectorAll('[data-qa]').forEach(b=>b.onclick=()=>{const key=b.dataset.qa;if(key==='products')window.psCleanRouteApi.memberProducts();else if(key==='tools')window.psOpenPlanTools();else if(key==='donation')window.psQa.openDashboardCardCopy(document.querySelector('#panelGrid .card'));else window.psQa.showAccountCenter(key)});
