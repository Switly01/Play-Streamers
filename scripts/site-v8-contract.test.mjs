...
test('site 10 assets are cache-busted and use fluid monochrome glass', async () => {
  ...
  assert.match(html, /play-streamers-build" content="2026-09-03-site-10\.33\.0"/);
  assert.match(html, /site-v7\.css\?v=10\.33\.0/);
  assert.match(html, /app\.js\?v=5\.11\.0/);
  assert.match(html, /site-v7\.js\?v=10\.33\.0/);
  assert.match(html, /app-final\.js\?v=5\.30\.0/);
  ...
  assert.match(html, /live-i18n\.js\?v=10\.9\.0/);
...
test('SW Bot audits deterministically and translation generation is release-only', async () => {
  ...
  assert.match(worker, /site-v7\.css\?v=10\.33\.0/);
  assert.match(worker, /site-v7\.js\?v=10\.33\.0/);
  ...
  assert.match(worker, /locales\/fr\.json\?v=2026-09-03\.1/);
...
test('versioned locale catalogs cover public, account, support and legal surfaces', async () => {
  ...
  for (const language of ['en', 'de', 'es', 'fr', 'ru', 'ar', 'ja']) {
    ...
    assert.equal(catalog.version, '2026-09-03.1');
